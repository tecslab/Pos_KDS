...
vi.mock("../../../../../../lib/order-on-the-way/server", () => ({
  createOrderOnTheWayService: dependencies.createService,
}));
...
beforeEach(() => {
  ...
  dependencies.createService.mockReset().mockReturnValue({
    markOnTheWay: dependencies.markOnTheWay,
  });
  dependencies.markOnTheWay.mockReset().mockResolvedValue({
    ok: true,
    value: {
      orderId,
      status: "ON_THE_WAY",
...
describe("PATCH /api/v1/delivery/orders/:orderId", () => {
  it("marks On the Way for a permission-authorized actor with path-owned input", async () => {
    ...
    expect(dependencies.markOnTheWay).toHaveBeenCalledWith(actorId, {
      orderId,
      sourceIp: null,
    });
  });
  ...
  it.each([
    ["INVALID_ON_THE_WAY_TRANSITION", 422],
    ["NOT_FOUND", 404],
    ["ORDER_NOT_READY", 409],
  ] as const)("maps %s to %s", async (code, status) => {
    dependencies.markOnTheWay.mockResolvedValue({
      ok: false,
      error: { kind: "order-on-the-way-error", code },
    });
    const response = await PATCH(new Request("http://localhost"), context());
    expect(response.status).toBe(status);
  });
  ...
  it("sanitizes operation, composition, and publication failures", async () => {
    dependencies.markOnTheWay.mockRejectedValue(
