...
vi.mock("../../../../../../../lib/order-delivered/server", () => ({
  createOrderDeliveredService: dependencies.createService,
}));
...
beforeEach(() => {
  ...
  dependencies.createService.mockReset().mockReturnValue({
    markDelivered: dependencies.markDelivered,
  });
  dependencies.markDelivered.mockReset().mockResolvedValue({
    ok: true,
    value: {
      orderId,
      status: "DELIVERED",
...
describe("PATCH /api/v1/delivery/orders/:orderId/delivered", () => {
  it("marks Delivered for a permission-authorized actor with path-owned input", async () => {
    ...
    expect(dependencies.markDelivered).toHaveBeenCalledWith(actorId, {
      orderId,
      sourceIp: null,
    });
  });
  ...
  it.each([
    ["INVALID_DELIVERED_TRANSITION", 422],
    ["NOT_FOUND", 404],
    ["ORDER_NOT_ON_THE_WAY", 409],
  ] as const)("maps %s to %s", async (code, status) => {
    dependencies.markDelivered.mockResolvedValue({
      ok: false,
      error: { kind: "order-delivered-error", code },
    });
    const response = await PATCH(new Request("http://localhost"), context());
    expect(response.status).toBe(status);
  });
  ...
  it("sanitizes operation, composition, and publication failures", async () => {
    dependencies.markDelivered.mockRejectedValue(
