...
vi.mock("../../../../../lib/payment-report/server", () => ({
  createPaymentReportService: dependencies.createService,
}));
...
const request = () =>
  new Request(
    `http://localhost/api/v1/reports/payments?restaurantId=${restaurantId}&date=2026-09-06&timeZone=America%2FGuayaquil`,
  );
...
describe("GET /api/v1/reports/payments", () => {
  it("authorizes reports.view before reading persisted payment data", async () => {
  ...
  it("rejects missing, duplicate, and export parameters", async () => {
    ...
    expect(
      (
        await GET(
          new Request(
            `http://localhost/api/v1/reports/payments?restaurantId=${restaurantId}&date=2026-09-06`,
          ),
        )
      ).status,
    ).toBe(400);
  ...
  it("does not compose the service after reports.view denial", async () => {
  ...
  it("maps persistence authorization failures to a safe response", async () => {
