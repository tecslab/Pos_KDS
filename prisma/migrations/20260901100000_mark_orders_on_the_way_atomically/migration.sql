CREATE FUNCTION public.mark_order_on_the_way(
  actor_user_id uuid,
...
BEGIN
  IF actor_user_id IS NULL
    ...
    RAISE EXCEPTION 'On-the-Way transition is invalid' USING ERRCODE = '22023';
  END IF;
  ...
  IF audit_occurred_at < transaction_timestamp() - interval '5 minutes'
    ...
    RAISE EXCEPTION 'On-the-Way transition timestamp is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF audit_source_ip IS NOT NULL THEN
    ...
    EXCEPTION WHEN invalid_text_representation THEN
      RAISE EXCEPTION 'On-the-Way transition audit source IP is invalid'
        USING ERRCODE = '22023';
    END;
  END IF;
  ...
  WHERE application_user.id = actor_user_id
    ...
    AND permission.code = 'delivery.on_the_way.mark'
  FOR SHARE OF application_user, assignment, role_permission, permission;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'On-the-Way transition is unauthorized'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'ORDER_ON_THE_WAY_NOT_FOUND' USING ERRCODE = 'P0001';
  END IF;

  IF target_order.status <> 'READY' THEN
    RAISE EXCEPTION 'ORDER_ON_THE_WAY_NOT_READY' USING ERRCODE = 'P0001';
  END IF;

  handoff_time := GREATEST(
    audit_occurred_at,
    target_order.ready_at,
    target_order.updated_at + interval '1 microsecond'
  );

  UPDATE public.orders AS handed_off_order
  SET status = 'ON_THE_WAY',
      on_the_way_at = handoff_time,
      updated_at = handoff_time
  WHERE handed_off_order.restaurant_id = target_order.restaurant_id
    AND handed_off_order.id = target_order.id;
  ...
  INSERT INTO public.audit_events (
    ...
    jsonb_build_object(
      ...
      'notes', target_order.notes,
      'totalAmount', target_order.total_amount,
      'collectedById', actor_user_id,
      'readyAt', target_order.ready_at,
      'onTheWayAt', handoff_time,
...
REVOKE ALL ON FUNCTION public.mark_order_on_the_way(
  uuid, uuid, timestamptz, text
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.mark_order_on_the_way(
  uuid, uuid, timestamptz, text
) TO service_role;
