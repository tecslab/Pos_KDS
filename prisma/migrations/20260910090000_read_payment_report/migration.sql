CREATE INDEX customer_baskets_restaurant_created_at_idx
  ON public.customer_baskets(restaurant_id, created_at);
...
BEGIN
  ...
  balances_as_of_period_end AS (
    SELECT
      ...
      basket.total_amount AS basket_total,
      COALESCE(SUM(payment.amount) FILTER (
        WHERE payment.recorded_at < requested_period_end
    ...
    LEFT JOIN public.payments AS payment
      ON payment.restaurant_id = basket.restaurant_id
     AND payment.basket_id = basket.id
     AND payment.recorded_at < requested_period_end
    ...
    WHERE basket.restaurant_id = target_restaurant_id
      AND basket.created_at < requested_period_end
      AND cancellation.order_id IS NULL
    GROUP BY sale_order.id, sale_order.order_number, basket.id, basket.total_amount
  ),
  outstanding AS (
    SELECT balance.*,
