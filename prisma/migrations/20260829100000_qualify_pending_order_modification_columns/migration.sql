...
DECLARE
  ...
  matching_snapshot public.order_line_sale_snapshots%ROWTYPE;
  merged_quantity bigint;
  modification_occurred_at timestamptz;
...
BEGIN
  ...
  SELECT order_to_modify.*
  INTO target_order
  FROM public.orders AS order_to_modify
  WHERE order_to_modify.id = target_order_id
  FOR UPDATE;
  ...
  END IF;
  modification_occurred_at := GREATEST(
    audit_occurred_at,
    target_order.updated_at + interval '1 microsecond'
  );
  IF modification_occurred_at > transaction_timestamp() + interval '5 minutes' THEN
    RAISE EXCEPTION 'order modification timestamp is invalid' USING ERRCODE = '22023';
  END IF;
  ...
  LOOP
    ...
    IF operation_kind = 'add' THEN
      ...
      END IF;
      PERFORM 1
      FROM public.customer_baskets AS target_basket
      WHERE target_basket.restaurant_id = target_order.restaurant_id
        AND target_basket.id = basket_id_value
        AND target_basket.order_id = target_order.id
        AND target_basket.status = 'PENDING'
      FOR UPDATE;
    ...
    ELSE
      ...
      IF NOT FOUND OR current_snapshot.id IS DISTINCT FROM expected_snapshot_id THEN
        RAISE EXCEPTION 'ORDER_MODIFICATION_STALE_ORDER' USING ERRCODE = 'P0001';
      END IF;
      SELECT target_line.basket_id
      INTO basket_id_value
      FROM public.order_lines AS target_line
      WHERE target_line.id = line_id_value;
    END IF;
    ...
    IF final_unit_price_value < 0 OR final_unit_price_value > 9999999999.99 THEN
    ...
    LEFT JOIN public.order_line_removals AS removed
      ON removed.restaurant_id = line.restaurant_id
     AND removed.order_line_id = line.id
    WHERE line.restaurant_id = target_order.restaurant_id
      AND line.basket_id = basket_id_value
      AND removed.order_line_id IS NULL
      AND (operation_kind = 'add' OR line.id <> line_id_value)
      AND snapshot.product_version_id = product_version_id_value
      AND snapshot.selected_options = selected_options_value
      AND snapshot.removed_ingredients = removed_ingredients_value
      AND snapshot.observations IS NOT DISTINCT FROM observation_value
    ORDER BY line.created_at, line.id
    LIMIT 1
    FOR UPDATE OF line;

    IF matching_line_id IS NOT NULL THEN
      SELECT snapshot.*
      INTO matching_snapshot
      FROM public.order_lines AS line
      JOIN public.order_line_sale_snapshots AS snapshot
        ON snapshot.restaurant_id = line.restaurant_id
       AND snapshot.order_line_id = line.id
       AND snapshot.id = line.current_snapshot_id
      WHERE line.restaurant_id = target_order.restaurant_id
        AND line.id = matching_line_id;

      merged_quantity := matching_snapshot.quantity::bigint + quantity_value::bigint;
      IF merged_quantity > 2147483647
        OR matching_snapshot.revision_number = 2147483647 THEN
        RAISE EXCEPTION 'order line quantity or revision is invalid' USING ERRCODE = '22023';
      END IF;
      quantity_value := merged_quantity::integer;
    END IF;

    line_total_value := final_unit_price_value * quantity_value;
    IF line_total_value > 9999999999.99 THEN
      RAISE EXCEPTION 'order line monetary value is invalid' USING ERRCODE = '22023';
    END IF;

    new_snapshot_id := gen_random_uuid();
    IF operation_kind = 'add' AND matching_line_id IS NULL THEN
    ...
    ELSIF matching_line_id IS NOT NULL THEN
      new_line_id := matching_line_id;
      new_revision := matching_snapshot.revision_number + 1;
    ELSE
      IF current_snapshot.revision_number = 2147483647 THEN
        RAISE EXCEPTION 'order line revision is invalid' USING ERRCODE = '22023';
      END IF;
      new_line_id := line_id_value;
      new_revision := current_snapshot.revision_number + 1;
    END IF;
    ...
    INSERT INTO public.order_line_sale_snapshots (
      ...
      removed_ingredients_value, observation_value, modification_occurred_at
    );

    IF matching_line_id IS NOT NULL OR operation_kind = 'replace' THEN
      UPDATE public.order_lines AS line_to_update
      SET current_snapshot_id = new_snapshot_id
      WHERE line_to_update.restaurant_id = target_order.restaurant_id
        AND line_to_update.id = new_line_id;
    END IF;

    IF operation_kind = 'replace' AND matching_line_id IS NOT NULL THEN
      INSERT INTO public.order_line_removals (
        order_line_id, restaurant_id, removed_snapshot_id, removed_by_id, removed_at
      ) VALUES (
        line_id_value,
  ...
  UPDATE public.orders AS order_to_update SET total_amount = (
  ...
  WHERE order_to_update.id = target_order.id;
  ...
  INSERT INTO public.audit_events (
    ...
    actor_user_id, modification_occurred_at, 'order.updated', 'order', target_order.id::text,
