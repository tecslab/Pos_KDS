...
DECLARE
  ...
  baskets_value jsonb;
  resale_movement record;
  inventory_movement_id_to_insert uuid;
...
BEGIN
  ...
  CREATE TEMP TABLE pg_temp.order_confirmation_resale_movements (
    inventory_item_id uuid PRIMARY KEY,
    quantity bigint NOT NULL,
    unit_of_measure text NOT NULL,
    sale_snapshot_ids uuid[] NOT NULL
  ) ON COMMIT DROP;
  ...
  LOOP
    SELECT
      ...
      version.resale_inventory_item_id
    INTO catalog_line
    ...
    SET product_name = catalog_line.name,
        ...
        resale_inventory_item_id = catalog_line.resale_inventory_item_id
  ...
  INSERT INTO pg_temp.order_confirmation_resale_movements (
    inventory_item_id, quantity, unit_of_measure, sale_snapshot_ids
  )
  SELECT
    item.id,
    sum(line.quantity),
  ...
  JOIN public.inventory_items AS item
    ON item.restaurant_id = target_restaurant_id
   AND item.id = line.resale_inventory_item_id
  WHERE line.resale_inventory_item_id IS NOT NULL
  GROUP BY item.id, item.unit_of_measure;
  ...
  IF EXISTS (
    ...
    WHERE line.resale_inventory_item_id IS NOT NULL
      AND NOT EXISTS (
        SELECT 1
        FROM pg_temp.order_confirmation_resale_movements AS movement
        WHERE movement.inventory_item_id = line.resale_inventory_item_id
      )
  ) THEN
    RAISE EXCEPTION 'ORDER_CONFIRMATION_STALE_CONFIGURATION'
      USING ERRCODE = 'P0001';
  ...
  IF EXISTS (
    SELECT 1
    FROM pg_temp.order_confirmation_resale_movements
    WHERE quantity > 99999999999
  ) THEN
  ...
  JOIN pg_temp.order_confirmation_resale_movements AS movement
    ON movement.inventory_item_id = item.id
  ...
  IF EXISTS (
    ...
    WHERE item.type IS DISTINCT FROM 'RESALE_ITEM'
  ...
  FOR resale_movement IN
    SELECT *
    FROM pg_temp.order_confirmation_resale_movements
    ORDER BY inventory_item_id
  LOOP
    inventory_movement_id_to_insert := gen_random_uuid();
    BEGIN
      INSERT INTO public.inventory_movements (
        ...
        inventory_movement_id_to_insert,
        target_restaurant_id,
        resale_movement.inventory_item_id,
        'SALE',
        -resale_movement.quantity::numeric(14, 3),
        resale_movement.unit_of_measure,
        actor_user_id,
        confirmation_time,
        'SALE',
        order_id_to_insert,
    ...
    EXCEPTION WHEN check_violation THEN
      ...
      IF inventory_error_message = 'inventory movement would produce a negative balance' THEN
        RAISE EXCEPTION 'ORDER_CONFIRMATION_INSUFFICIENT_INVENTORY'
          USING ERRCODE = 'P0001';
      END IF;
      RAISE;
    END;
    ...
    INSERT INTO public.audit_events (
      ...
      'inventory_movement',
      inventory_movement_id_to_insert::text,
      NULL,
      jsonb_build_object(
        'inventoryMovementId', inventory_movement_id_to_insert,
        'restaurantId', target_restaurant_id,
        'inventoryItemId', resale_movement.inventory_item_id,
        'type', 'SALE',
        'quantityDelta', -resale_movement.quantity::numeric(14, 3),
        'unitOfMeasure', resale_movement.unit_of_measure,
        'recordedById', actor_user_id,
        'recordedAt', confirmation_time,
        'businessOriginType', 'SALE',
        'businessOriginId', order_id_to_insert,
        'comments', NULL,
        'reversedMovementId', NULL,
        'saleSnapshotIds', to_jsonb(resale_movement.sale_snapshot_ids)
      ),
      source_ip_value
    );
  END LOOP;
