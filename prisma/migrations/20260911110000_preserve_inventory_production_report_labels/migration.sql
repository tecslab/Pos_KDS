ALTER TABLE public.inventory_movements
  ADD COLUMN inventory_item_name_snapshot text NOT NULL
  DEFAULT 'Unattributed historical inventory item';

ALTER TABLE public.production_batches
  ADD COLUMN recipe_name_snapshot text NOT NULL
  DEFAULT 'Unattributed historical recipe',
  ADD COLUMN output_inventory_item_id_snapshot uuid,
  ADD COLUMN output_inventory_item_name_snapshot text NOT NULL
  DEFAULT 'Unattributed historical inventory item';

UPDATE public.production_batches AS batch
SET output_inventory_item_id_snapshot = COALESCE(
  (
    SELECT movement.inventory_item_id
    FROM public.inventory_movements AS movement
    WHERE movement.restaurant_id = batch.restaurant_id
      AND movement.business_origin_type = 'PRODUCTION'
      AND movement.business_origin_id = batch.id
      AND movement.type = 'PRODUCTION_OUTPUT'
    ORDER BY movement.recorded_at, movement.id
    LIMIT 1
  ),
...
WHERE version.restaurant_id = batch.restaurant_id
  AND version.id = batch.recipe_version_id;

ALTER TABLE public.production_batches
  ALTER COLUMN output_inventory_item_id_snapshot SET NOT NULL;

CREATE FUNCTION public.capture_inventory_movement_item_name_snapshot()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  SELECT item.name
  INTO STRICT NEW.inventory_item_name_snapshot
...
CREATE TRIGGER inventory_movements_capture_item_name_snapshot
BEFORE INSERT ON public.inventory_movements
FOR EACH ROW EXECUTE FUNCTION public.capture_inventory_movement_item_name_snapshot();

CREATE FUNCTION public.capture_production_batch_report_snapshots()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public, pg_temp
AS $$
BEGIN
  SELECT recipe.name, recipe.output_inventory_item_id, item.name
  INTO STRICT NEW.recipe_name_snapshot,
    NEW.output_inventory_item_id_snapshot,
    NEW.output_inventory_item_name_snapshot
  FROM public.recipe_versions AS version
  JOIN public.recipes AS recipe
    ON recipe.restaurant_id = version.restaurant_id
   AND recipe.id = version.recipe_id
...
CREATE TRIGGER production_batches_capture_report_snapshots
BEFORE INSERT ON public.production_batches
FOR EACH ROW EXECUTE FUNCTION public.capture_production_batch_report_snapshots();
...
BEGIN
  ...
  movements_json AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'inventory_item_name', movement.inventory_item_name_snapshot,
  ...
  purchases_json AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'lines', (
        ...
        ) ORDER BY movement.inventory_item_name_snapshot, line.id), '[]'::jsonb)
  ...
  day_production_batches AS (
    SELECT batch.*, version.version_number
  ...
  production_batches_json AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'recipe_name', batch.recipe_name_snapshot,
      'output_inventory_item_id', batch.output_inventory_item_id_snapshot,
      'output_inventory_item_name', batch.output_inventory_item_name_snapshot,
  ...
  adjustments_json AS (
    ...
    JOIN public.inventory_movements AS movement
      ON movement.restaurant_id = adjustment.restaurant_id
     AND movement.inventory_item_id = adjustment.inventory_item_id
     AND movement.id = adjustment.inventory_movement_id
  ...
  waste_records_json AS (
    ...
    JOIN public.inventory_movements AS movement
      ON movement.restaurant_id = waste.restaurant_id
     AND movement.inventory_item_id = waste.inventory_item_id
     AND movement.id = waste.inventory_movement_id
